...
export class EnhancedAutoApplyWorkflowService {
  ...
  async startEnhancedAutoApplyProcess(request: EnhancedJobApplicationRequest): Promise<{
    ...
    try {
      ...
      try {
        ...
        console.warn('⚠️ Browser automation unavailable, using simulation mode');
        return await this.simulateApplicationProcess(request, sessionId);
      }
      ...
      if (request.profile.enableAICoverLetter && jobDetails.jobDescription) {
        try {
          generatedCoverLetter = await this.geminiService.generateCoverLetter(
            jobDetails.jobTitle,
      ...
      return {
        success: true,
        sessionId,
        message: `🚀 Enhanced auto-apply workflow started! Job: ${jobDetails.jobTitle} at ${jobDetails.companyName}. Review email sent${whatsappSent ? ' and WhatsApp notification sent' : ''}. Please approve within 60 seconds.`
      };
      ...
      return {
        success: false,
        message: `❌ Enhanced auto-apply failed: ${(error as Error).message}`,
      };
    } finally {
  ...
  private async extractJobDetails(page: Page, jobUrl: string): Promise<{
    ...
    try {
      ...
      const details = await this.geminiService.extractJobDetails(pageContent, jobUrl);
      return {
      ...
      for (const selector of mapping.selectors) {
        try {
          ...
          if (await field.isVisible({ timeout: 1000 })) {
            ...
            console.log(`✅ Filled ${mapping.key}: ${mapping.value}`);
            break;
          }
        } catch (e) {
          // Continue to next selector
      ...
      for (const selector of submitSelectors) {
        try {
          ...
          if (await submitButton.isVisible({ timeout: 2000 })) {
            ...
            submitted = true;
            console.log(`✅ Application submitted via ${method}`);
            break;
          }
        } catch (e) {
          // Continue
  ...
  private async simulateApplicationProcess(request: EnhancedJobApplicationRequest, sessionId: string): Promise<{
    success: boolean;
    sessionId?: string;
    message: string;
  }> {
    try {
      ...
      const jobDetails = await this.simulateJobDetailsExtraction(request.jobUrl);
      
      // Update session with simulated job details
      await storage.updateApplicationSession(sessionId, {
        jobTitle: jobDetails.jobTitle,
        company: jobDetails.companyName,
      });
      ...
      const simulatedFormData = {
        ...
        coverLetterUploaded: !!request.coverLetterFile || !!generatedCoverLetter,
        simulationMode: true
      };

      // Update session with simulated form data
      await storage.updateApplicationSession(sessionId, {
      ...
      const session = await storage.getApplicationSessionById(sessionId);
      if (session) {
        await this.emailService.sendReviewEmail(session, simulatedFormData);
        
        // Start auto-submit timer
        this.startAutoSubmitTimerSimulation(sessionId);
      }

      return {
        success: true,
        sessionId,
        message: 'Application prepared in simulation mode. Browser automation unavailable but email review system active.'
      };
      ...
      return {
        success: false,
        message: 'Application process failed: ' + (error as Error).message
      };
    }
  }

  private async simulateJobDetailsExtraction(jobUrl: string): Promise<{
    jobTitle: string;
    companyName: string;
    jobDescription: string;
  }> {
    // Extract basic details from URL
    const url = new URL(jobUrl);
    const hostname = url.hostname;
    
    let companyName = 'Unknown Company';
    if (hostname.includes('google.com')) companyName = 'Google';
    else if (hostname.includes('microsoft.com')) companyName = 'Microsoft';
    else if (hostname.includes('amazon.com')) companyName = 'Amazon';
    else if (hostname.includes('meta.com') || hostname.includes('facebook.com')) companyName = 'Meta';
    else companyName = hostname.split('.')[0];

    return {
      jobTitle: 'Software Engineer Position',
      companyName,
      jobDescription: `Software engineering position at ${companyName}. This role involves developing high-quality software solutions, collaborating with cross-functional teams, and contributing to innovative projects.`
    };
  }

  private async startAutoSubmitTimerSimulation(sessionId: string) {
    // Set 60-second timer for auto-submit
    setTimeout(async () => {
      try {
        const session = await storage.getApplicationSessionById(sessionId);
        ...
        console.log(`⏰ Auto-submitting application (simulation) for session ${sessionId}`);
        await this.submitApplicationSimulation(sessionId, 'auto');
      } catch (error) {
        console.error('Auto-submit simulation error:', error);
      }
    }, 60000); // 60 seconds
  }

  async submitApplicationSimulation(sessionId: string, method: 'auto' | 'manual'): Promise<void> {
    try {
      ...
      const session = await storage.getApplicationSessionById(sessionId);
      if (session) {
        await this.sendConfirmationNotifications(session, method, true);
      }
      ...
      await storage.createApplicationLog({
        ...
        jobUrl: session?.jobUrl || '',
        result: 'success',
        notes: `Submitted via ${method} (simulation mode)`,
      });

      console.log(`✅ Application submitted via ${method} (simulation mode)`);

    } catch (error) {
      console.error('Application submission simulation error:', error);
